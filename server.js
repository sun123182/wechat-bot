...
// ======== 企业微信配置（与控制台一致） ========
const WECHAT_TOKEN =
  process.env.WECOM_TOKEN || 'IQCLf5VMl31IenTIoPk6953';
const WECHAT_ENCODING_AES_KEY =
  process.env.WECOM_ENCODING_AES_KEY ||
  'x62C4zUbz8kGWunRHkN8m3t9nyDkzO8zELS3AtcWQ7f';
...
app.post('/callback', async (req, res) => {
  try {
    ...
    // 智能机器人 POST 过来的通常是 JSON：{"encrypt":"..."}
    // 优先按 JSON 解析拿 encrypt，失败再按 XML 兼容模式解析
    let encrypt = null;
    try {
      const jsonBody = JSON.parse(rawBody);
      encrypt = jsonBody.encrypt || jsonBody.Encrypt;
      console.log('[POST] parsed JSON body, got encrypt:', !!encrypt);
    } catch (e) {
      console.log('[POST] body is not JSON, try XML');
    }

    if (!encrypt) {
      const parsedXml = await parseXml(rawBody);
      encrypt = parsedXml?.xml?.Encrypt;
    }

    if (!encrypt) {
      console.error('[POST] no Encrypt field after JSON/XML parse');
